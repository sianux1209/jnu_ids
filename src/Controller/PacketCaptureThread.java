...
public class PacketCaptureThread extends Thread {
		...
		while (true) {
			...
			if (info != null) {
				...
				System.out.print("packet_type : " + tmp.getPacketResource("packet_type") + "\nsrc_ip : "
						+ tmp.getPacketResource("src_ip") + "\nsrc_mac : " + tmp.getPacketResource("src_mac")
						+ "\nsrc_port : " + tmp.getPacketResource("src_port") + "\ndst_ip : "
						+ tmp.getPacketResource("dst_ip") + "\ndst_mac : " + tmp.getPacketResource("dst_mac")
						+ "\ndst_port : " + tmp.getPacketResource("dst_port") + "\narrival_time : "
						+ tmp.getPacketResource("arrival_time") + "\nprotocol_number : "
						+ tmp.getPacketResource("protocol_number") + "\nprotocol_name : "
						+ tmp.getPacketResource("protocol_name") + "\nevent_number : "
						+ tmp.getPacketResource("event_number") + "\nevent_name : "
						+ tmp.getPacketResource("event_name") + "\nrisk : " + tmp.getPacketResource("risk\n"));
				...
				mainView.tableModel.insertRow(0,setTableRow(tmp));
				
				
				//mainView.tableModel.setDataVector(dataVector, columnIdentifiers);
				
				
				//IPTree 셋팅
							
					nodeSet.add(tmp.getPacketResource("src_ip"));
					setTreeNode();
	...
	private Object[] setTableRow(Packet_Table packet) {
		...
		String tempPacket[] = {
				packet.getPacketResource("risk"),
				packet.getPacketResource("arrival_time"),
				packet.getPacketResource("protocol_name"),
				packet.getPacketResource("src_ip"),
				packet.getPacketResource("dst_ip"),
				packet.getPacketResource("event_name"),
		};
		...
		if (pack instanceof TCPPacket) {
			...
			if (pack.data.length > 0) { // 데이터가 있을 경우 http
				...
				if (((TCPPacket) pack).src_ip != null)
					tmp.setPacket("src_ip", ((TCPPacket) pack).src_ip.toString().substring(1,
							(((TCPPacket) pack).src_ip.toString().length())));
				tmp.setPacket("src_port", Integer.toString(((TCPPacket) pack).src_port));
				if (((TCPPacket) pack).dst_ip != null)
					tmp.setPacket("dst_ip", ((TCPPacket) pack).dst_ip.toString().substring(1,
							(((TCPPacket) pack).dst_ip.toString().length())));
				...
				if (tmp.getPacketResource("dst_port").compareTo("80") == 0
						| tmp.getPacketResource("src_port").compareTo("80") == 0) {
			...
			if (pack.data.length > 0) {
				...
				tmp.setPacket("arrival_time", d_format.format((Calendar.getInstance().getTime())));
				tmp.setPacket("src_ip", ((UDPPacket) pack).src_ip.toString().substring(1,
						(((UDPPacket) pack).src_ip.toString().length())));
				tmp.setPacket("src_port", Integer.toString(((UDPPacket) pack).src_port));
				tmp.setPacket("dst_ip", ((UDPPacket) pack).dst_ip.toString().substring(1,
						(((UDPPacket) pack).dst_ip.toString().length())));
			...
			if (pack.data.length > 0) {
				...
				tmp.setPacket("arrival_time", d_format.format((Calendar.getInstance().getTime())));
				tmp.setPacket("src_ip", ((ICMPPacket) pack).src_ip.toString().substring(1,
						(((ICMPPacket) pack).src_ip.toString().length())));
				tmp.setPacket("dst_ip", ((ICMPPacket) pack).dst_ip.toString().substring(1,
						(((ICMPPacket) pack).dst_ip.toString().length())));
			...
			if (pack.data.length > 0) {
				...
				tmp.setPacket("arrival_time", d_format.format((Calendar.getInstance().getTime())));
				tmp.setPacket("src_ip", ((IPPacket) pack).src_ip.toString().substring(1,
						(((IPPacket) pack).src_ip.toString().length())));
				tmp.setPacket("dst_ip", ((IPPacket) pack).dst_ip.toString().substring(1,
						(((IPPacket) pack).dst_ip.toString().length())));
